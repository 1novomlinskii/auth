...
permission:
  ...
  bash:
    "*": ask
  skill:
    golang-toolchain: allow
    golang-conventions: allow
    golang-project-structure: allow
    golang-error-handling: allow
    golang-concurrency: allow
    golang-testing: allow
    golang-patterns: allow
    golang-codegen: allow
    golang-dependencies: allow
    golang-libraries: allow
    dba-sql-guide: allow
    "*": deny
---
...
НЕ читай `instruction/golang/` напрямую. Вместо этого **активируй нужный skill** по задаче:
